...
poch1_legendre(int i, double x)
{
    if(i == 0) 
	   ...
	   return ( 2 * i - 1) * legendre_rek(i - 1, x) / i + (2 * i - 1) * x * poch1_legendre(i - 1, x) / i - (i - 1) * poch1_legendre(i - 2, x) / i;
...
poch2_legendre( int i, double x)
{
    if(i == 0) 
	   ...
	   return 2 * ( 2 * i - 1) * poch1_legendre(i - 1, x) / i + (2 * i - 1) * x * poch2_legendre(i - 1, x) / i- (i - 1) * poch2_legendre(i - 2, x) / i;
...
poch3_legendre(int i, double x)
{
    if(i == 0) 
	   ...
	   return 3 * ( 2 * i - 1) * poch2_legendre(i - 1, x) / i + (2 * i - 1) * x * poch3_legendre(i - 1, x) / i - (i - 1) * poch3_legendre(i - 2, x) / i;
